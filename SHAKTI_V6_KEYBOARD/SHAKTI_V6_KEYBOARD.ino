...
#define LEFT 68 //ASCII Value for LEFT Button
#define RIGHT 67 //ASCII Value for RIGHT Button
char button;
...
void setup() {
  ...
  Set_Origin(55,30); // To Set the Orgin 
...
void Make_Axis() // To Make X-axis and Y-axis
{
...
void Make_Axis_Max() // To Make X-axis and Y-axis for Zoomed screen
{
...
void Legends_Update_X_Max()
{
  ...
  my_lcd.Set_Text_Back_colour(0x0000);
  my_lcd.Set_Text_Size(2);
  my_lcd.Print_Number_Int(long((my_lcd.Get_Width()-x_origin)/10),legend_x_max+160,legend_y_max, 3,' ', 10);
}
...
void Legends_Update_Y_Max()
{
  ...
  my_lcd.Set_Text_Back_colour(0x0000);
  my_lcd.Set_Text_Size(2);
  my_lcd.Print_Number_Int(long((Scale_exp_max*max_limit_y)/10),legend_x_max+160,legend_y_max+18, 3,' ', 10);
}
...
void Record_Data(uint16_t adc_value) // Recording the data everytime a interrupt is generated
{
  ...
  if( adc_value >=550 && adc_value <= 600)
  {
  record_data[adc_value] = record_data[adc_value]+3; // The record data array with 1024 elements are increased accordingly depending on what the ADC value is
  }
  else
  {
...
void Make_Max_Value() // To print the max valued channel
{
 ...
 else if(Screen3)
 {
  ...
  my_lcd.Set_Text_Back_colour(0x0000);
  my_lcd.Set_Text_Size(2);
  my_lcd.Print_String(text_max_value,legend_x_max-100,legend_y_max); // This is to only print string "MAX CPS" the value is printed in Update_Max_Value
  Update_Max_Value(); // Print the channel that has the maximum value
 }
}
...
void Update_Max_Value() // This is to print or update the channel that has max value
{
  ...
  {
  my_lcd.Print_Number_Int(long(max_channel),legend_x_max-100,legend_y_max+18, 4,' ', 10); // Prints it on the screen
  }
}
