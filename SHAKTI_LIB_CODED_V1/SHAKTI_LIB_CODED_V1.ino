...
void setup() {
  ...
  Set_Max_Limit(1024,500); // To Set Max x-axis limit and y-axis limit
...
for(int count=0; count< my_lcd.Get_Width()-x_origin; count++) // To draw the spectrum lines starting from origin to the end of the screen width
{
  ...
  my_lcd.Draw_Fast_VLine(x_origin+count+1,my_lcd.Get_Height()-y_origin-((record_data_compressed[count]*my_lcd.Get_Height()-y_origin)/(500*Scale)),((record_data_compressed[count]*my_lcd.Get_Height()-y_origin)/(500*Scale))); // Drawing the Light Blue Line
  my_lcd.Set_Draw_color(0x0000); // Black Colour
  my_lcd.Draw_Fast_VLine(x_origin+count+1,0,my_lcd.Get_Height()-y_origin-((record_data_compressed[count]*my_lcd.Get_Height()-y_origin)/(500*Scale))); // Drawing the reamining Black Line i.e not light blue
}
...
void Record_Data(uint16_t adc_value) // Recording the data everytime a interrupt is generated
{
  ...
  if(record_data_compressed[adc_value] >= 100*Scale) // This is to update the Spectrum when the height reaches the specified value. In this case 100*Scale
  {
